...
def group_lipids(df, mode='Headgroup'):
    ...
    unparsed_lipids = []
    lipid_meta = {}  # lipid -> dict(headgroup, carbons, db, mods)

    # --- helpers for PL mode
    PL_SET    = {'PA', 'PC', 'PE', 'PG', 'PI', 'PS'}
    LYSO_SET  = {'LPC', 'LPE', 'LPI', 'LPS'}
    NEUTRAL   = {'DG', 'TG', 'MG'}

    def normalize_hg(hg: str) -> str:
        if hg is None:
            return None
        # x = hg.upper().replace('-', '').strip()
        x = hg
        # common synonyms
        if x == 'DAG': x = 'DG'
        if x == 'TAG': x = 'TG'
    ...
    for lipid in lipid_names:
        ...
        for part in parts:
            ...
            m = base_pattern.fullmatch(part)
            if m:
                parsed = True
                hg, c, db, mod = m.groups()
                headgroup = hg
                total_carbons = int(c)
            ...
            m = ether_lipid_pattern.match(part)
            if m:
                parsed = True
                hg, c, db = m.groups()
                headgroup = hg
                ...
                total_double_bonds = int(db)
                if ";" in part:
                    modifications.extend(part.split(";")[1:])
                modifications.append("O-ether")
                break

            m = plasmalogen_pattern.match(part)
            if m:
                parsed = True
                hg, c, db = m.groups()
                headgroup = hg
            ...
            m = sterol_pattern.match(part)
            if m:
                parsed = True
                hg, c, db, mods = m.groups()
                headgroup = hg
                total_carbons = int(c)
                total_double_bonds = int(db)
            ...
            m = base_pattern.fullmatch(cleaned)
            if m:
                parsed = True
                hg, c, db, mod = m.groups()
                headgroup = hg
        ...
        headgroup = normalize_hg(headgroup)
        lipid_meta[lipid] = {
            'headgroup': headgroup,
    ...
    # 2) special PL mode: exclusive combined buckets
    if mode == 'PL':
        for lipid, meta in lipid_meta.items():
            hg = meta['headgroup']
            if hg in PL_SET:
                grouped_lipids['PL'].append(lipid)
            elif hg in LYSO_SET:
                grouped_lipids['lysoPL'].append(lipid)
            elif hg in NEUTRAL:
                grouped_lipids[hg].append(lipid)  # DG/TG/MG individually
            else:
                grouped_lipids['Other'].append(lipid)
        return dict(grouped_lipids), unparsed_lipids

    # 3) all other modes: compute dynamic bins ONCE after parsing
    carbon_map = {lip: meta['carbons'] for lip, meta in lipid_meta.items()}
    if len(carbon_map) >= 3:
        carbon_bins = bin_carbons_dynamic(carbon_map)
    else:
        # avoid warning spam; just label as Unbinned
        carbon_bins = {lip: f"Unbinned ({meta['carbons']}C)" for lip, meta in lipid_meta.items()}
    ...
    for lipid, meta in lipid_meta.items():
        ...
        carbon_class = bin_carbons(total_carbons)
...
# ----------------------------------------------
def group_lipids_old(df, mode='Headgroup'):
